...
static HKHeroController *sharedInstance = nil;
...
{
	if (!sharedInstance) {
		sharedInstance = [[HKHeroController alloc] init];
	}
...
- (void)killApplication:(SBApplication *)app removeFromSwitcher:(BOOL)removeFromSwitcher
{	
	if(SYSTEM_VERSION_LESS_THAN(@"6.0")) {
		// iOS 5
...
{
	[self killApplication:app removeFromSwitcher:YES];
}
...
- (UIImage *)iconForApplicationWithDisplayID:(NSString *)displayID size:(int)size
{
	if(SYSTEM_VERSION_LESS_THAN(@"6.0")) {
